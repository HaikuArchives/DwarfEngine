...
{
public:
	Load3ds(Buffer& buffer, RawMesh* out);
	~Load3ds();
	void Load();
...
private:
	int32 GetString(char *);
	int32 ReadChunk(Chunk *);
	int32 ReadColorChunk(Chunk *, uint32 type);
	int32 ReadPercentChunk(Chunk *, uint32 type);

	int32 ProcessNextChunk(Chunk *);
	int32 ProcessNextObjectChunk(Chunk *);
	int32 ProcessNextMaterialChunk(Chunk *);
	int32 ProcessNextKeyFrameChunk(Chunk *);
	int32 FillVertexBuffer(Chunk *);
	int32 FillIndexBuffer(Chunk *);
	int32 FillTexCoordBuffer(Chunk *);
	int32 SortIndicesByMaterial(Chunk *);
	
	int32 ComputeNormals();
	int32 CompileObjects();
	int32 CleanUp();
	...
	int current_object;
	void * mBuffer;
	String m_name;	// name of the mesh
