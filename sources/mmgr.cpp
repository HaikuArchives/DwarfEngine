...
// DWARFENGINE SPECIFIC CHANGES
// only compile this if we are in debug mode
// These are beos only changes. They make the code nonfunctional in windows.
#ifndef WIN32
#include "Defs.h"
#endif

#ifdef _DEBUG // only include this in debug mode

//~DWARFENGINE SPECIFIC CHANGES

#pragma warning(push, 3) //**** I get a lot of warnings if I'm on level 4 from this file...
...
#ifdef	WIN32
	#ifdef	_DEBUG
	#define	m_assert(x, msg) if ((x) == false) __asm { int 3 }
	#else
	#define	m_assert(x, msg) {}
	#endif
#elif defined(__BEOS__)
	#ifdef _DEBUG
		extern void debugger(const char *message);
		#define	m_assert(x, msg) if ((x) == false) debugger(msg)
	#else
		#define m_assert(x, msg) {}
	#endif
#else	// Linux uses assert, which we can use safely, since it doesn't bring up a dialog within the program.
	#define	m_assert(cond, msg) assert(cond)
#endif
...
static	void	log(const char *format, ...)
{
	...
	m_assert(fp, "The memory logger is unable to log information to a file (can't open the file for some"
	"reason.) You can interrogate the variable 'buffer' to see what was supposed to be logged (but won't be.)");
...
static	void	dumpLeakReport()
{
	...
	// some reason.)
	m_assert(fp, "The memory report generator is unable to log information to a file (can't open the file for some reason.)");
	if (!fp) return;
...
bool	&m_breakOnRealloc(void *reportedAddress)
{
	...
	m_assert(au != NULL, "You tried to set a breakpoint on reallocation for an address that doesn't exist. Interrogate the"
	"stack frame or the variable 'au' to see which allocation this is.");
	...
	m_assert(au->allocationType == m_alloc_malloc ||
		 ...
		 au->allocationType == m_alloc_realloc, "You tried to set a breakpoint on reallocation for an address that wasn't allocated in a way that is compatible with reallocation.");
...
bool	&m_breakOnDealloc(void *reportedAddress)
{
	...
	// stack frame or the variable 'au' to see which allocation this is.
	m_assert(au != NULL, "You tried to set a breakpoint on deallocation for an address that doesn't exist. Interrogate the stack frame or the variable 'au' to see which allocation this is.");
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		m_assert(currentAllocationCount != breakOnAllocationCount, "You requested a breakpoint on a specific allocation count.");
		...
		if (!reservoir)
		{
			...
			// allocations
			m_assert(reservoir != NULL, "The memory manager failed to allocate internal memory for tracking the allocations");
		...
		m_assert(reservoir != NULL, "Logical flow says this should never happen...");
		...
		if (au->allocationNumber == 315)
		{
//			m_assert(0, "alloc 315 encountered");
		}
		//~TODO: remove this evil code
		...
		m_assert(au->actualAddress != NULL, "The requested allocation simply failed (you're out of memory). Interrogate the variable 'au' or the stack frame to see what you were trying to do.");
		#endif
		...
		m_assert(allocationType != m_alloc_unknown, "This allocation was made from a source that isn't setup to use this memory tracking software, use the stack frame to locate the source and include our H file.");
...
void	*m_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		...
		m_assert(au != NULL, "You tried to reallocate RAM that wasn't allocated by this memory manager.");
		...
		m_assert(m_validateAllocUnit(au), "The allocation unit that is about to be reallocated is damaged. But you probably already know that from a previous assert you should have seen in validateAllocUnit() :)");
		...
		m_assert(reallocationType != m_alloc_unknown, "This reallocation was made from a source that isn't setup to use this memory tracking software, use the stack frame to locate the source and include our H file.");
		...
		m_assert(au->allocationType == m_alloc_malloc ||
			 au->allocationType == m_alloc_calloc ||
			 au->allocationType == m_alloc_realloc, "You were trying to reallocate RAM that was not allocated in a way that is compatible with realloc. In other words, you have a allocation/reallocation mismatch.");
		...
		// allocation unit.
		m_assert(au->breakOnRealloc == false, "the \"break on realloc\" flag for this allocation unit is set (and will continue to be set until you specifically shut it off. Interrogate the 'au' variable to determine information about this allocation unit.");
		...
		m_assert(newActualAddress, "the requested allocation simply failed (you're out of memory) Interrogate the variable 'au' "
			"to see the original allocation. You can also query 'newActualSize' to see the amount of memory trying to be allocated. "
			"Finally, you can query 'reportedSize' to see how much memory was requested by the caller.");
		#endif
		...
		m_assert(m_validateAllocUnit(au), "Something went wrong, because the allocation unit was properly validated PRIOR to the reallocation. This should not happen.");
...
void	m_deallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	try
	{
		...
		m_assert(au != NULL, "You tried to deallocate RAM that wasn't allocated by this memory manager.");
		...
		// already know that from a previous assert you should have seen in validateAllocUnit() :)
		m_assert(m_validateAllocUnit(au), "The allocation unit that is about to be deallocated is damaged. But you probably already know that from a previous assert you should have seen in validateAllocUnit() :)");

		// If you hit this assert, then this deallocation was made from a source that isn't setup to use this memory
		// tracking software, use the stack frame to locate the source and include our H file.
		m_assert(deallocationType != m_alloc_unknown, "This deallocation was made from a source that isn't setup to use this memory tracking software, use the stack frame to locate the source and include our H file.");
		...
		m_assert((deallocationType == m_alloc_delete       && au->allocationType == m_alloc_new      ) ||
			 ...
			 "You were trying to deallocate RAM that was not allocated in a way that is compatible with the deallocation method requested. In other words, you have a allocation/deallocation mismatch.");
		...
		m_assert(au->breakOnDealloc == false, "The \"break on dealloc\" flag for this allocation unit is set. Interrogate the 'au' variable to determine information about this allocation unit.");
...
bool	m_validateAllocUnit(const sAllocUnit *allocUnit)
{
	...
	for (unsigned int i = 0; i < paddingSize; i++, pre++, post++)
	{
		...
		m_assert(*pre == (long) prefixPattern, "You should know that this allocation unit has been damaged. Something (possibly the owner?) has underrun the allocation unit (modified a few bytes prior to the start). You can interrogate the variable 'allocUnit' to see statistics and information about this damaged allocation unit.");
		...
		m_assert(*post == (long) postfixPattern, "You should know that this allocation unit has been damaged. Something (possibly the owner?) has overrun the allocation unit (modified a few bytes after the end). You can interrogate the variable 'allocUnit' to see statistics and information about this damaged allocation unit.");
	}
...
bool	m_validateAllAllocUnits()
{
	...
	// assertion.
	m_assert(allocCount == stats.totalAllocUnitCount, "he internal memory (hash table) used by this memory tracking software is damaged! "
	"The best way to track this down is to use the alwaysLogAll flag in conjunction with STRESS_TEST macro to narrow in on the offending "
	"code. After running the application with these settings (and hitting this assert again), interrogate the memory.log file to find the "
	"previous successful operation. The corruption will have occurred between that point and this assertion.");
	...
	m_assert(errors == 0, "You've probably already been notified that there was a problem with a allocation unit in a prior call to validateAllocUnit(), but this assert is here just to make sure you know about it. :)");
