...
float Player::Mul()
{
	return (255.0 - (float)step[seqcount].damp) / 255.0;
}
...
bool Player::NextBuffer()
{
	if (nexts < 1){
		...
		if (currstep->eff){
			...
			// PWM
			case 0x40 : ip->SetPWMOffs((currstep->eff & 0xf)/16.1); break;

			// filter
			case 0x70 :	cutoffstep = 1.0-((currstep->eff & 0xf)+1)/256.0;break;
			case 0x80 :	cutoffstep = 1.0+((currstep->eff & 0xf)+1)/256.0;break;
			case 0x90 : cutoffstep = 1.0; cutoff = ((currstep->eff & 0xf)+1)/16.0;cutoff *= cutoff;break;

			// amp
			case 0xa0 :	ampstep = -((currstep->eff & 0xf)+1)/256.0; break;
			case 0xb0 :	ampstep = ((currstep->eff & 0xf)+1)/256.0; break;
			case 0xc0 : ampstep=0; amp = (currstep->eff & 0xf)/15.0; break;
