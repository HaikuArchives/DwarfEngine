...
public:
		...
		m_skybox(RenderingDevice::QUADS, 8)
	{
		...
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(40, -40, 130), Point(0, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(40, 40, 130), Point(1, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(-40, 40, 130), Point(1, 0)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(-40, -40, 130), Point(0, 0)));

/*
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(80, 40, -40), Point(0, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(80, 40, 40), Point(1, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(80, -40, 40), Point(1, 0)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(80, -40, -40), Point(0, 0)));
*/
/*
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(-40, 40, 80), Point(0, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(40, 40, 80), Point(1, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(40, -40, 80), Point(1, 0)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(-40, -40, 80), Point(0, 0)));
*/
/*
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(-40, 80, 40), Point(0, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(40, 80, 40), Point(1, 1)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(40, 80, -40), Point(1, 0)));
		poly2.AddItem(Portal::poly_vertex(Vertex<3>(-40, 80, -40), Point(0, 0)));
*/

		GenericPortal portal(Object(Vertex<3>(0,0,100), Quaternion(Euler(0,D_PI*0.1,0)), sector), poly2);
		...
		poly.AddItem(Portal::poly_vertex(Vertex<3>(-40, 40, 40), Point(0, 1)));
		poly.AddItem(Portal::poly_vertex(Vertex<3>(40, 40, 40), Point(1, 1)));
		poly.AddItem(Portal::poly_vertex(Vertex<3>(40, -40, 40), Point(1, 0)));
		poly.AddItem(Portal::poly_vertex(Vertex<3>(-40, -40, 40), Point(0, 0)));
		MirrorPortal mirror(poly);
		mirror.SetShader(HShader(TEXT("mirror")));
		m_mirror = mirror;
		AddPortal(&m_mirror);
...
{
public:
	Model(const Object &obj):Entity(obj), m_time(0)
	{
		Directory dir(TEXT("../data"));
		m_mesh = HMesh(new Mesh(&dir, TEXT("impsym.3ds")));
...
public:
	Particle()
	{
		...
		m_spline.add_key(first, 0, 0, 0, 0);
		float time = 0.0f;
		for (int32 i = 1; i < 10; i++)
		{
			time += ((float)rand())/((float)RAND_MAX) * 10.0f;
			m_spline.add_key(Vertex<3>(Rand(), Rand(), Rand()), 0, 0, 0, time);
		}
		m_spline.add_key(first, 0, 0, 0, time + 1);
		m_duration = time + 1;
...
private:
	...
	virtual void Draw(RenderingDevice &device, const Camera &frustum) const
	{
		device.SetStates(m_state);
	...
	TCBSpline<Vertex<3> >::const_iterator m_iterator;
	float m_duration;
...
//	PakFile dir(TEXT("data.zip"));
	...
	sector.spawn(name::Model, Vertex<3>(0, 0, 50));
